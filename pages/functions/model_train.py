...
#def get_differencing_order(close_price):
    ...
    #while True:
        #if  p_value > 0.05:
            #d += 1
            #close_price = close_price.diff().dropma()
            #p_value = stationary_check(close_price)
    #else:
        #break
    #return d
...
def evaluate_model(original_price, diffrencing_order):
    train_data, test_data  = original_price[:-30], original_price[-30:]
    predictions = fit_model(train_data, diffrencing_order)
    #rmse = np.sqrt(mean_squared_error(test_data, predictions))
    rmse = np.sqrt(mean_squared_error(test_data[-30:], predictions))
